...
namespace HandNote.Data.Repositories.User.Commands
{
    public static class CreateUserCommand
    {
        ...
        public static async Task<OperationResult<UserAuthenticationData>> ExecuteAsync(
            ...
            ILogger logger, JwtSettings jwtSettings)
        {
            if (dto == null)
            {
            ...
            try
            {
                ...
                return await ProcessResultAsync(reader, logger, dto, jwtSettings);
            }
            ...
            {
                logger.LogError(ex, "Unexpected error during user creation for {Username} , Error : {Error}", dto.UserName, ex.Message);
        ...
        private static async Task<OperationResult<UserAuthenticationData>> ProcessResultAsync(
            ...
            ILogger logger,
            UserRegistrationDto dto, JwtSettings jwtSettings)
        {
            if (!await reader.ReadAsync())
            {
                logger.LogWarning("No result returned from registration procedure for {Username}", dto);
            ...
            if (status != "SUCCESS")
            {
                ...
                logger.LogWarning("Registration failed for {Username}: {Message} (Error: {ErrorNumber})",
                    dto.UserName, message, errorNumber);
