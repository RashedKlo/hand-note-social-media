...
namespace HandNote.Services.Friendship
{
    public sealed class FriendshipService : IFriendshipService
    {
        ...
        public async Task<OperationResult<FriendshipCreateResponseDto>> SendFriendRequestAsync(FriendshipCreateRequestDto dto)
        {
            ...
            var validationResult = await FriendshipValidation.ValidateFriendRequestCreationAsync(dto, _friendshipRepository, _logger);
        ...
        public async Task<OperationResult<FriendshipUpdateResponseDto>> AcceptFriendRequestAsync(FriendshipUpdateRequestDto dto)
        {
            ...
            dto.NewStatus = EnFriendshipStatus.Accepted;
            ...
            var validationResult = FriendshipValidation.ValidateFriendshipStatusUpdateAsync(dto, _friendshipRepository, _logger);
            if (!validationResult.IsSuccess)
            {
        ...
        public async Task<OperationResult<FriendshipUpdateResponseDto>> DeclineFriendRequestAsync(FriendshipUpdateRequestDto dto)
        {
            ...
            dto.NewStatus = EnFriendshipStatus.Declined;
        ...
        public async Task<OperationResult<FriendshipUpdateResponseDto>> CancelFriendRequestAsync(FriendshipUpdateRequestDto dto)
        {
            ...
            dto.NewStatus = EnFriendshipStatus.Cancelled;
        ...
        public async Task<OperationResult<FriendshipUpdateResponseDto>> BlockUserAsync(FriendshipUpdateRequestDto dto)
        {
            ...
            dto.NewStatus = EnFriendshipStatus.Blocked;
        ...
        public async Task<OperationResult<List<UserFriendsGetResponseDto>>> GetUserFriendsAsync(UserFriendsGetRequestDto dto)
        {
            if (dto.UserId <= 0)
            {
                _logger.LogWarning("Get user friends attempted with invalid UserId: {UserId}", dto.UserId);
                return OperationResult<List<UserFriendsGetResponseDto>>.Failure("UserId must be a positive integer");
            }

            _logger.LogInformation("Processing get user friends for UserId: {UserId}", dto.UserId);

            var friendsResult = await _friendshipRepository.GetUserFriendsAsync(dto);

            if (!friendsResult.IsSuccess)
            {
                _logger.LogWarning("Get user friends failed for UserId {UserId}: {Message}",
                    dto.UserId, friendsResult.Message);
                return OperationResult<List<UserFriendsGetResponseDto>>.Failure(friendsResult.Message);
            }

            _logger.LogInformation("Get user friends completed successfully - UserId: {UserId}, Friends Count: {FriendsCount}",
                dto.UserId, friendsResult.Data?.Count ?? 0);

            return OperationResult<List<UserFriendsGetResponseDto>>.Success(friendsResult.Data!, friendsResult.Message);
        }
    }
}