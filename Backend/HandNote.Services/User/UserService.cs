...
namespace HandNote.Services.User
{
    public sealed class UserService : IUserService
    {
        ...
        public async Task<OperationResult<GoogleUserAuthenticationData>> AuthenticateWithGoogleAsync(string email)
        {
            if (string.IsNullOrEmpty(email))
            {
                _logger.LogWarning("Authentication attempted with empty email");
                return OperationResult<GoogleUserAuthenticationData>.Failure($"Authenticated failed for email : {email} ");
            }

            _logger.LogInformation("Processing user google authenticate for email: {Email}", email);

            var authResult = await _userRepository.AuthenticateUserWithGoogle(email);

            if (!authResult.IsSuccess)
            {
                _logger.LogWarning("Login failed for email: {Email}: {Message}", email, authResult.Message);
                return OperationResult<GoogleUserAuthenticationData>.Failure(authResult.Message);
            }

            _logger.LogInformation("User logged in successfully - UserId: {UserId}",
                authResult.Data?.User?.UserId);

            return OperationResult<GoogleUserAuthenticationData>.Success(authResult.Data!, authResult.Message);
        }
